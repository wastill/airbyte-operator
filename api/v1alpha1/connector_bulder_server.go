...
type ConnectorBuilderServerSpec struct {
	...
	RoleConfig *RoleGroupConnectorBuilderServerSpec `json:"roleConfig,omitempty"`

	// +kubebuilder:validation:Optional
	RoleGroups map[string]*RoleGroupConnectorBuilderServerSpec `json:"roleGroups,omitempty"`
}

func (connector *ConnectorBuilderServerSpec) InitRoleGroups() {
	for _, role := range connector.RoleGroups {
		role.InitRoleGroup(connector.RoleConfig)
	}
}

func (connectorRole *RoleGroupConnectorBuilderServerSpec) InitRoleGroup(roleConfig *RoleGroupConnectorBuilderServerSpec) {
	outConfig := roleConfig
	inConfig := connectorRole
...
func (connector *ConnectorBuilderServerSpec) InitRoleConfig(clusterConfig *ClusterConfigSpec) {
	outConfig := clusterConfig
	inConfig := connector.RoleConfig
	if inConfig.Replicas == 0 {
		inConfig.Replicas = AirbyteBootloaderReplicas
	}
	if inConfig.Annotations == nil {
